# ============================================================
# MODULE — High-level curation wrapper
#
# DESTINATION: R/curate.R
#
# This module provides rb_curate_reference(), the main
# user-facing function that orchestrates the entire
# curation pipeline:
#
#   1. QC Pass 1
#   2. Optional ML classification
#   3. QC Pass 2
#   4. Taxonomy joining
...
# ------------------------------------------------------------
# Internal helper: Run all QC checks on a data frame
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Main wrapper: rb_curate_reference()
# ------------------------------------------------------------

#' Curate a reference sequence database
#'
#' @param data A standardized data frame containing at least
#'   `species`, `sequence`, `seq_type`, and `source`.
#' @param blast_db Path to a pre-built BLAST database for contaminant screening.
#' @param numt_fasta Path to a FASTA file of known NUMTs.
#' @param run_classifier Logical. If TRUE, trains an ML classifier to predict
#'   markers for sequences labeled as "other" or "other_unknown".
#' @param run_divergence Logical. If TRUE, runs phylogenetic divergence checks
#'   (requires mafft and FastTree installed).
#' @param taxonomy_table Optional data frame for taxonomic lineage joining.
#' @param db_path Optional path to write the final SQLite database.
#'
#' @return A list containing `qc1`, `classifier`, `qc2`, and `final_data`.
#' @export
